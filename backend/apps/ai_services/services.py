...
class AIService:
    ...
    @staticmethod
    def normalize_category(raw, description):
        """Map the raw LLM reply onto one of the allowed categories.

        Falls back to keyword rules when the reply matches none of them,
        so junk output never becomes a Category row.
        """
        raw = (raw or '').strip()
        if raw in AIService.ALLOWED_CATEGORIES:
            return raw
        for name in AIService.ALLOWED_CATEGORIES:
    ...
    def categorize_transaction(description, amount):
        ...
        try:
            ...
            category = AIService.normalize_category(
